...
RSpec.describe RichTextExtraction::Helper do
  let(:helper) { Class.new { include RichTextExtraction::Helper }.new }

  describe '#extract_links' do
    it 'extracts links from text' do
      text = 'Check out https://example.com and http://test.org'
      result = helper.extract_links(text)
    ...
    it 'handles nil input' do
      result = helper.extract_links(nil)
      expect(result).to be_empty
    end
  end
  ...
  describe '#extract_mentions' do
    it 'extracts mentions from text' do
      ...
      result = helper.extract_mentions(text)
    ...
    it 'handles nil input' do
      result = helper.extract_mentions(nil)
      expect(result).to be_empty
    end
  end
  ...
  describe '#extract_hashtags' do
    it 'extracts hashtags from text' do
      ...
      result = helper.extract_hashtags(text)
    ...
    it 'handles nil input' do
      result = helper.extract_hashtags(nil)
      expect(result).to be_empty
    end
  end
  ...
  describe '#validate_url' do
    it 'validates valid URLs' do
      ...
      valid_urls.each do |url|
        expect(helper.validate_url(url)).to be true
      end
    end
    ...
    it 'rejects invalid URLs' do
      ...
      invalid_urls.each do |url|
        expect(helper.validate_url(url)).to be false
      end
    end

    it 'handles nil input' do
      expect(helper.validate_url(nil)).to be false
    end
  end
  ...
  describe '#extract_all' do
    it 'extracts all types of content from text' do
      ...
      result = helper.extract_all(text)
  ...
  describe 'module inclusion' do
    it 'can be included in any class' do
      test_class = Class.new do
        include RichTextExtraction::Helper
      end
      
      instance = test_class.new
      expect(instance).to respond_to(:extract_links)
      expect(instance).to respond_to(:extract_mentions)
      expect(instance).to respond_to(:extract_hashtags)
      expect(instance).to respond_to(:validate_url)
      expect(instance).to respond_to(:extract_all)
    end
  end

  describe 'error handling' do
    it 'handles malformed input gracefully' do
      expect { helper.extract_links('') }.not_to raise_error
      expect { helper.extract_mentions('') }.not_to raise_error
      expect { helper.extract_hashtags('') }.not_to raise_error
      expect { helper.validate_url('') }.not_to raise_error
      expect { helper.extract_all('') }.not_to raise_error
    end

    it 'handles very long input' do
      long_text = 'a' * 10000 + ' https://example.com ' + 'b' * 10000
      result = helper.extract_links(long_text)
  ...
  describe 'performance' do
    it 'handles multiple extractions efficiently' do
      ...
      100.times { helper.extract_all(text) }
